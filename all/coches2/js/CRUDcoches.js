...
/*function buscar(array, valor) {
  ...
  while (i < arrayEscuderias.length && !encontrado) {
    indice = arrayEscuderias[i].array.findIndex(
      (elemento) => elemento.dni === valor
    );
    if (indice != -1) {
      elementoEncontrado = arrayEscuderias[i].array[indice];
      encontrado = true;
      //indice = i;
    }
    i++;
...
function verPilotos() {
  ...
  while (i < arrayEscuderias.length && !encontrado) {
    ...
    console.log("index piloto:", indexDnipiloto);
    if (indexDnipiloto != -1) {
      pilotoEncontrado = arrayEscuderias[i].arrayPilotos[indexDnipiloto];
      encontrado = true;
      console.log("i", i);
 ...
 // pilotoEncontrado = buscar(element.dni arrayPilotos, idPiloto);
  ...
  if (indexDnipiloto == -1) {
    ...
    console.log("piloto encontrado: ", pilotoEncontrado);
...
function bajaMecanicos() {
  ...
  if (indexEliminar == -1) {
    ...
    console.log(escuderiaEncontrada);
    arrayEscuderias[escuderiaEncontrada].arrayMecanicos.splice(
      indexEliminar,
      1
    );
